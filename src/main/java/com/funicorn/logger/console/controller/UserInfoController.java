...
public class UserInfoController {
     ...
     */
    @PostMapping("/login")
    public Result<ContextUserInfo> login(@Validated LoginDTO loginDTO){
        ...
        ContextUserInfo contextUserInfo = JsonUtil.object2Object(userInfo,ContextUserInfo.class);
        contextUserInfo.setUserId(userInfo.getId());
        HttpSession session = ((ServletRequestAttributes) Objects.requireNonNull(RequestContextHolder.getRequestAttributes())).getRequest().getSession();
        session.setAttribute(WebRequestFilter.USER_INFO_KEY, contextUserInfo);
        session.setMaxInactiveInterval(1800);
        return Result.ok(contextUserInfo);
    }
    ...
    public Result<?> editNickname(@RequestBody UserDTO userDTO){
        if (UserType.Admin!=UserType.valueOf(ContextUtil.getContextUser().getUserType())
                && !userDTO.getUserId().equals(ContextUtil.getContextUser().getUserId())) {
            return Result.error("非管理员用户，无操作权限");
        }
